import 'package:chumanter/views/quize_view/quiz_question_shopping_profile.dart';
...
class QuizeViewShoppingProfile extends StatefulWidget {
  QuizeViewShoppingProfile({
    super.key,
    this.isTimer = true,
    this.viewMode,
  ...
  @override
  State<QuizeViewShoppingProfile> createState() =>
      _QuizeViewShoppingProfileState();
}

class _QuizeViewShoppingProfileState extends State<QuizeViewShoppingProfile> {
  final _pageController = PageController();
  int? _currentPage;
  List<Map<String, dynamic>> questionItems = [];
  List<UserResponse?> userResponses = List.generate(8, (index) => null);
  ...
  void initState() {
    ...
    _currentPage = 1;
    questionItems = QuizDataShopping.getShoppingQuestions();
  ...
  void loadShoppingOptions() async {
        ...
        child: Column(
          children: [
                                      ...
                                      child: FadeInAnimation(
                                        child: InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            height: MySize.scaleFactorHeight * 57,
                                            width: MySize.screenWidth,
                                            decoration: BoxDecoration(
            ...
            paddingComp(ButtonWidget(
                onTap: onTapSubmit,
                text: _currentPage == 8&&!widget.viewMode! ? "Submit".tr() : _currentPage==8&&widget.viewMode! ? "Close".tr() : "Next".tr()
            )),
            SizedBox(
              height: MySize.scaleFactorHeight * 10,
            ),
  ...
  void saveSelectedOptions(List<UserResponse?> userResponses) async {
    ...
    for (int i = 0; i < userResponses.length; i++) {
      final selectedOption = userResponses[i]?.selectedOptionText;
      prefs.setString('shopping_question_$i', selectedOption ?? '');
  ...
  void updateCompletionPercentage()async{
    ...
    Provider.of<CalculateProfileCompletionPercent>(context, listen: false)
        .setShoppingCompletionPercent(completionPercentage);
  ...
  async {
    ...
    preff.setBool('completedShopping', true);
  ...
  Future<bool> submitShoppingProfile() async {
    SharedPreferences preff = await SharedPreferences.getInstance();
    try {
      var uid = preff!.getString('uid');

      var response = await post(Uri.parse(AppUrls.submitShoppingProfile), body: {
        "userid": uid ?? '',
        // Using the null-aware operator to handle a potential null value
        "grocery": '${userResponses[0]?.selectedOptionText}',
        "onlineshop": '${userResponses[1]?.selectedOptionText}',
        "shopping_type": '${userResponses[2]?.selectedOptionText}',
        "groceries_from": '${userResponses[3]?.selectedOptionText}',
        "store": '${userResponses[4]?.selectedOptionText}',
        "brand_con": '${userResponses[5]?.selectedOptionText}',
        "monthly_grocery": '${userResponses[6]?.selectedOptionText}',
        "brands_online": userResponses[7]?.selectedOptionText == "Depends on what's required" ?
        "Depends on whats required" : userResponses[7]?.selectedOptionText == "Doesn't Matter" ? "Doesnt Matter"
        : userResponses[7]?.selectedOptionText,
      });
      ...
      if (response.statusCode == 200) {
        CommonFuncs.apiHitPrinter(
            ...
            "userid: $uid\n"
                ...
                "brand_con: ${userResponses[5]?.selectedOptionText}\n"
                "monthly_grocery: ${userResponses[6]?.selectedOptionText}\n"
                "brands_online: ${userResponses[7]?.selectedOptionText}\n",
            dataa,
  ...
  void onTapSubmit() async {
    ...
    if (userResponses[_currentPage! - 1] != null) {
      ...
      if(_currentPage == 8 && widget.viewMode!)
        {
          Navigator.pop(context);
        }
        ...
        for (var response in userResponses) {
          if (kDebugMode) {
            print('Question ${response!.questionIndex + 1}:${response.selectedOptionText}\n'
              'Question text: ${questionItems[response!.questionIndex]['question']}');
          }
        }

        bool isSuccess = await submitShoppingProfile();
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextWidget(
                          text: 'Thank you for completing Shopping Profile. Please complete other profiles too'.tr(),
                          textAlign: TextAlign.center,
                          maxLines: 6,
                          fontSize: MySize.size18,
                          fontFamily: AppConst.primaryFont,
